...
<template>
  <section>
    <uvCollapseRightSlot title="数据变换" not-collapse>
      <template #slot-content>
        <a-row :gutter="32">
          <a-col :span="12">
            <div class="flex flex-col gap-5">
              <a-tabs v-model:activeKey="activeKey" class="left-box" @change="changeTab"
                >>
                ...
                <a-tab-pane key="3" tab="谱图平滑" class="!leading-[4.5]">
                  ...
                  <div class="flex items-center gap-4">
                    ...
                    <a-input-number
                      class="flex-1"
                      v-model:value="formState.coefficient"
                      :min="0"
                      :step="1"
                      :precision="0"
                    />
                ...
                <a-tab-pane key="4" tab="谱图微分" class="!leading-[4.5]">
                  ...
                  <div class="flex items-center gap-4">
                    ...
                    <a-input-number
                      class="flex-1"
                      v-model:value="formState.coefficientFen"
                      :min="0"
                      :step="1"
                      :precision="0"
                    />
...
// 定义所有 API 函数的参数类型
type ApiParams = {
  advancedOperationsPort: Record<string, any> // 高级运算接口
  differentialPort: Record<string, any> // 谱图微分接口
  fourOperationsPort: Record<string, any> // 四则运算接口
  luminanceModeConversionPort: Record<string, any> // 光度模式转换接口
  methodParameter: Record<string, any> // 方法参数接口
  queryMeasurementParameters: Record<string, any> // 查询源测量数据接口
  saveTask: Record<string, any> // 保存任务接口
  smoothnessPort: Record<string, any> // 谱图平滑接口
  exportTaskWord: Record<string, any> // 导出任务Word接口
  downloadDocTimeSweep: Record<string, any> // 导出任务Word接口
  downloadDocSpectrumScanning: Record<string, any> // 导出任务Word接口
  downloadDocConciseSpectrumScanning: Record<string, any> // 导出任务Word接口
  downloadDocConciseTimeSweep: Record<string, any> // 导出任务Word接口
}
const props = defineProps({
  ...
  api: {
    ...
    validator(value: Record<string, any>) {
      ...
      ].every((key) => key in value && typeof value[key] === 'function')
    },
  },
  measureType: {
    type: String,
  ...
  switch (key) {
    case '1': // 四则运算
      Object.assign(formState, {
        measureDataId1: undefined,
        arithmeticOperators: undefined,
        dataType: '0',
        measureDataId2: undefined,
        analysisDataName: '',
      })
      chartData.value = []
      break
    ...
    case '5': // 光度模式转换
      Object.assign(formState, {
        measureDataIdHuan: undefined,
        dataTypeHuan: '1',
        analysisDataNameHuan: '',
      })
      chartDataHuan.value = []
      break
  }
...
const queryMeasurementParametersFun = async () => {
  const { code, data } = await props.api.queryMeasurementParameters({
...
const fourOperationsPortFun = async () => {
  const { code, data } = await props.api.fourOperationsPort({
...
const advancedOperationsPortFun = async () => {
  const { code, data } = await props.api.advancedOperationsPort({
...
const smoothnessPortFun = async () => {
  const { code, data } = await props.api.smoothnessPort({
...
const differentialPortFun = async () => {
  const { code, data } = await props.api.differentialPort({
...
const luminanceModeConversionPortFun = async () => {
  const { code, data } = await props.api.luminanceModeConversionPort({
...
const methodParameterFun = async (methodId: string) => {
  ...
  if (code === 200) {
    if (props.measureType === '光谱扫描') {
      ...
      xShowRange.value.min = 0
      xShowRange.value.max =
        data.scanUnit === '1' ? Number(data.scanTime) + 5 : (Number(data.scanTime) + 1) * 60
    }
  }
}
// 保存
