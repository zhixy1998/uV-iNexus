  ...
  <div class="uv-edit-cell-table">
    <div class="toolbar">
      <a-space>
        ...
        <a-input-number size="small" :min="1" :max="99" v-model:value="rowNum" class="!w-12" />
...
import columnConfig from '@/views/experiment/lumneScence/components/columnConfig.vue'
...
// 下拉保存单元格内容
const saveSelectCell = (event: string, rowKey: string, colKey: string) => {
  editingCell.rowKey = rowKey
  editingCell.colKey = colKey
...
const updateConfigHeaders = (configHeaders: Column[]) => {
  // columns.value = cloneDeep(
  //   columnsCopy
  //     .slice(0, columnsCopy.length - 1)
  //     .concat(
...
const addRow = () => {
  ...
  // 定量和光度默认样品编号带default-  其他测量都是纯数字
  let startIndex
  const defaultSampleNumber = 'default-'
  if (props.measureType === '光度测量' || props.measureType === '定量测定') {
    const defaultSampleList = dataSource.value.filter((item) =>
      item.sampleNumber.startsWith('default-'),
    )
    startIndex = defaultSampleList?.length
      ? Number(
          defaultSampleList[defaultSampleList.length - 1].sampleNumber.replace(
            defaultSampleNumber,
            '',
          ),
        )
    ...
    startIndex =
      dataSource.value && dataSource.value.length
        ? Number(dataSource.value[dataSource.value.length - 1].sampleNumber)
        : 0
  }
  ...
  for (let i = 0; i < rowNum.value; i++) {
    const newRowIndex = String(startIndex + i + 1)
    const newRow: { key: string; [key: string]: any } = { key: newRowIndex }
    columns.value.forEach((col: Column) => {
      if (col.key !== 'action') newRow[col.key] = ''
    })

    // 使用 defaultNum.value 作为起始编号
    if (hasSampleNumber) {
      newRow['sampleNumber'] =
        props.measureType === '光度测量' || props.measureType === '定量测定'
          ? `${defaultSampleNumber}${startIndex + i + 1}`
          : `${startIndex + i + 1}`
    }
    if (hasSampleName) {
      newRow['sampleName'] = defaultName.value ? `${defaultName.value}${startIndex + i + 1}` : ''
    }
...
const pasteData = async () => {
  try {
    ...
    console.log('Using internal clipboard instead of system clipboard')
  }
