...
<template>
  <a-tabs v-model:activeKey="activeKey">
    <a-tab-pane v-if="methodsParams.coefficientMode !== 'direct'" key="0" tab="标准样品">
      <uvEditCellTable
        ...
        :isParallelSamples="true"
        v-model:initialData="tableDataBiao"
        :defaultSampleType="'stand'"
        :measureType="`定量测定`"
...
const getSampleList = async (sampleKind: any) => {
  if (props.taskId) {
    const { code, data } = await sampleApi.value.getQuantitySampleList({
    ...
    if (code === 200)
      if (sampleKind === '0') {
        tableDataBiao.value = data.map((item: any) => ({
          ...item,
          key: item.sampleId,
          sampleType: item.sampleType ?? 'stand',
        }))
      } else {
        tableDataUnknow.value = data.map((item: any) => ({
          ...item,
          key: item.sampleId,
          sampleType: item.sampleType ?? 'unknown',
        }))
      }
  }
}
const querySampleHeadsFun = async (sampleKind: any) => {
  if (props.taskId) {
    const { code, data } = await sampleApi.value.queryQuantitySampleHeads({
      sampleKind: sampleKind,
      taskId: props.taskId,
    })
    if (code === 200) {
...
const transformColumnData = (columnData: any, activeKey: string) => {
  ...
  columnArr.forEach((item: any) => {
    if (item.dataIndex === 'sampleType') {
      ...
      item.defaultValue = activeKey === '0' ? 'stand' : 'unknown'
...
const isRepeat = computed(() => {
  if (tableDataBiao.value && tableDataBiao.value.length && activeKey.value === '0') {
    return tableDataBiao.value.some((item: any, index: number) => {
      return (
        tableDataBiao.value.findIndex((item2: any) => item2.sampleNumber === item.sampleNumber) !==
        index
      )
    })
  } else if (tableDataUnknow.value && tableDataUnknow.value.length && activeKey.value === '1') {
    return tableDataUnknow.value.some((item: any, index: number) => {
      return (
        tableDataUnknow.value.findIndex(
          (item2: any) => item2.sampleNumber === item.sampleNumber,
        ) !== index
      )
    })
  }
