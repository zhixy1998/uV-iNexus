...
const queryMeasurementParametersFun = async () => {
  const { code, data } = await queryMeasurementParameters({
...
const fourOperationsPortFun = async () => {
  const { code, data } = await fourOperationsPort({
...
const advancedOperationsPortFun = async () => {
  const { code, data } = await advancedOperationsPort({
...
const smoothnessPortFun = async () => {
  const { code, data } = await smoothnessPort({
...
const differentialPortFun = async () => {
  const { code, data } = await differentialPort({
...
const luminanceModeConversionPortFun = async () => {
  const { code, data } = await luminanceModeConversionPort({
...
const methodParameterFun = async (methodId: string) => {
  ...
  if (code === 200) {
    xShowRange.min = Number(data.endWaveLength)
    xShowRange.max = Number(data.startWaveLength)
  }
}
// 保存
...
const spectrumExportWordFun = async () => {
  const res = await spectrumExportWord({
    taskId: props.taskId ? props.taskId : '',
  })
