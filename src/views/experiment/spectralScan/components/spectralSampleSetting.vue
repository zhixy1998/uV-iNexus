...
import { addOrDeleteSamplePort } from '@/api'
import uvModal from '@/components/uvModal/index.vue'
import sampleTemplate from '@/views/experiment/lumneScence/components/sampleTemplate.vue'
...
const addSamplePortFun = async () => {
  ...
  const { code, data } = await addOrDeleteSamplePort(APIParams)
  if (code == 200) {
