              ...
              >
                <template #title="{ data }">
                  <div class="flex justify-between gap-4">
                    <div>{{ data.title }}</div>
                    <div class="flex items-center gap-1" v-if="data.level !== 0">
                      <ColorInput
                      ...
                      />
                      <SyncOutlined
                        v-if="data.type === '0' && data.level === 1"
                        :spin="finished === '1' && measureKey === data.key"
                        @click.stop="measureSingleIsRepeat(data)"
                      />
                      <SyncOutlined
                        v-if="data.type === '0' && data.level === 2 && measureKey === data.key"
                        :spin="finished === '1' && measureKey === data.key"
                      />
                    </div>
                  </div>
...
const treeData = ref<TreeNode[]>([
  {
    title: '样品列表',
    key: '0-0',
    level: 0, // 添加level来判断是否显示颜色和删除，第一级永远会有测量
    children: [],
  },
  {
    title: '变化列表',
...
const isReapeat = ref(false) //判断方法是不是单池重复
const checkIsDelete = () => {
...
const methodParameterFun = async (methodId: string) => {
  ...
  if (code === 200) {
    ...
    isReapeat.value = data.repetitionPattern === '单池重复'
  }
}
// 测量单条数据
const measureSingleIsRepeat = (data: any) => {
  if (!isReapeat.value) {
    // 单条数据非重复
    measureSingle(data)
  } else {
    // 单条数据重复
    measureSingleRepeat(data)
  }
}
// 测量单条数据重复
const measureSingleRepeat = (data: any) => {
  if (finished.value === '1') {
    message.error('测量过程进行中！')
    return
  }
  // TODO: 1. 获取到当前测量的键所在数据结构的对象,创建一个对象数据添到children数组中，并设置对象key为当前测量键
  const node = findNodeByKey(data.key)
  console.log('node', node, treeData.value)
  const length = node?.children.length || 0
  node?.children.unshift({
    title: `${node.title}(${length + 1})`,
  ...
  checkedKeys.value = [...checkedKeys.value, `${data.key}-${length + 1}`]
  activeKey.value = `${data.key}-${length + 1}`
  measureKey.value = `${data.key}-${length + 1}`
 ...
 * @param tree
 */
const getNonFirstLevelTitles = (tree: TreeNode[] = treeData.value) => {
  ...
  const traverse = (nodes: TreeNode[], level: number) => {
    for (const node of nodes) {
      if (
        ...
        checkedKeys.value.includes(node.key)
      ) {
...
const getChartLineData = (tree: TreeNode[]) => {
  ...
  const traverse = (nodes: TreeNode[], level: number) => {
    for (const node of nodes) {
      if (level > 1 && node.children?.length === 0 && checkedKeys.value.includes(node.key)) {
        chartLines.push({
          name: node.title,
          data: node.data,
          color: node.color,
...
const handleMessage = async (event: MessageEvent) => {
  ...
  if (!isReapeat.value) {
    ...
    // 单池重复
    const keyData = `${measureKey.value.split('-')[0]}-${data.key}`
    console.log(keyData, measureKey.value, '45655')
    if (keyData !== measureKey.value) {
      const node = findNodeByKey(measureKey.value.split('-')[0])
      node?.children.unshift({
        title: `${node.title}(${data.key})`,
        key: keyData,
        children: [],
        level: 2,
        type: '0',
      ...
      console.log('进入了几次显示一下', treeData.value, measureKey.value.split('-')[0])

      checkedKeys.value = [...checkedKeys.value, keyData]
...
const sampleListPortListFun = async () => {
  ...
  if (code == 200) {
    // TODO: 树形结构的每一个都添加数据包含树节点信息
    const treeSampleData = addLevelDataToTreeData(data)
    sampleList.value = treeSampleData
    treeData.value[0].children = treeSampleData
    // TODO: 获取数据后所有节点默认选中
...
const addLevelDataToTreeData = (treeData: TreeNode[], level = 1): TreeNode[] => {
  return treeData.map((node: TreeNode) => {
    const newNode: TreeNode = {
      ...node,
      level: level,
      measureType: 0,
      key: node.measureId || (node.sampleId as string),
      data: node.data || [],
      children: node.children ? addLevelDataToTreeData(node.children, level + 1) : [],
    }
    return newNode
  })
}
