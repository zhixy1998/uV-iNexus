...
import { methodParameter, queryEssentialInformation, useThisMethodPort } from '@/api'
...
const useThisMethods = async (messageInfo: any) => {
  const { code, data } = await useThisMethodPort({ methodId: messageInfo.id })
  if (code === 200) {
    emits('changeMethodsTask', {
      methodId: messageInfo.id,
      taskId: data.taskId,
...
const methodParameterFun = async (methodId: string) => {
  ...
  if (code === 200) {
    ...
    emits('methodId', code === 200 ? data.methodId : '')
    const arr = [
      {
        title: '基本信息',
        items: [
      ...
      },
      {
        ...
        items: [
          {
            label: '起始波长',
            value: methodsParams.startWaveLength ? `${methodsParams.startWaveLength}nm` : '-',
            width: '100%',
          },
          {
            label: '终止波长',
            value: methodsParams.endWaveLength ? `${methodsParams.endWaveLength}nm` : '-',
            width: '100%',
          },
          {
            label: '扫描间隔',
            value: methodsParams.waveLengthInterval ? `${methodsParams.waveLengthInterval}nm` : '-',
            width: '100%',
          },
        ],
      },
      {
        title: '重复模式',
        items: [
          { label: '自动重复', value: methodsParams.repetitionPattern },
          {
            label: '重复次数',
            value: methodsParams.repetitionTime ? `${methodsParams.repetitionTime}次` : '-',
          },
          {
            label: '时间间隔',
      ...
      },
      {
        title: '仪器参数',
        items: [
          { label: '扫描速度', value: methodsParams.scanSpeedName },
          {
            label: '光谱带宽',
    ...
    if (methodsParams.repetitionPattern === '单池重复') {
      ...
      arr[2].items = [{ label: '自动重复', value: methodsParams.repetitionPattern }]
      descriptOptions.value = arr
    }
  }
}
