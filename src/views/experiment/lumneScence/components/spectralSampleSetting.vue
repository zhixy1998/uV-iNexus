...
import { addOrDeleteLumneSample, getLumneSampleList } from '@/api'
...
const getSampleListFun = async () => {
  ...
  const { code, data } = await getLumneSampleList(params)
  if (code == 200) {
...
const addOrDeleteSampleFun = async () => {
  ...
  const { code, data } = await addOrDeleteLumneSample(params)
  if (code == 200) {
