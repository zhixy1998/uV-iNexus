...
<template>
  <uvModal
    ...
    :okTitle="'保存'"
    :footer="props.formDisabled ? null : true"
  >
    <template #content>
      <lumneScenceParams
...
import { updateMethodParameter } from '@/api'
...
import lumneScenceParams from './lumneScenceParams.vue'
...
const saveOtherNewMethods = async () => {
  ...
  try {
    const { code, data } = await updateMethodParameter({
      ...formState.value,
      resultParams: methodsParamsRef.value.resultParams.map((item: any) => item.params),
      sampleInputParams: methodsParamsRef.value.sampleInputParams.map((item: any) => item.params),
      qcParams: methodsParamsRef.value.qcParams.map((item: any) => item.params),
      approxParams: methodsParamsRef.value.approxParams.map((item: any) => item.params),
